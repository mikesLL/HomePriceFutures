...
void load_pricepath(void *snodes_in, double rent_in, double ph0_in, double ret0_in, double csf_1yr_in, int t_id, string city_init_in, int city_id) {

	int s1, s2, s_test;                             // state in current period, state in next period
	int N_print =  10000;                          // number of observations to print to file
	//int N_sim = 200000;  // 1000000;                                            // number of simulations
...
double gamma0_store[] = {
	...
	double ph_nd_std[] = { -2.0, -1.5, -1.0, -0.5, 0.0, .5, 1.0, 1.5, 2.0 };     // stdev centers (tauchen-style discretization) 
	//double ph_nd_std[] = { -2.0, -1.0, 0.0, 1.0, 2.0 };
	//double ph_nd_std[] = {  -1.0, 0.0, 1.0 };     // stdev centers (tauchen-style discretization) 
	double rent_nd_std[] = { 0.0 }; //{ -1.0, 0.0, 1.0 };
	//double rent_nd_std[] = { -1.0, 0.0, 1.0 };
	//double yi_nd_std[] = { 0.0 };
	double yi_nd_std[] = {  -1.5, 0.0, 1.5 }; 
	//double yi_nd_std[] = { 0.0 };
	
	string city_init = city_init_in;
	...
	cout << "begin load_pricepath" << endl ;
    double duration;
	...
	//vector<vector<double>> yc_str(T_sim + 1, vector<double>(N_sim, 0.0));               // city-wide income
	vector<vector<double>> yi_str(T_sim + 1, vector<double>(N_sim, 0.0));               // individual income

	vector<double> ph_str1(N_sim, 0.0);              // store city-wide home prices
	vector<double> rent_str1(N_sim, 0.0);             // store city-wide rent
	vector<double> yc_str1(N_sim, 0.0);               // city-wide income
	vector<double> yi_str1(N_sim, 0.0);               // individual income

	vector<double> ph_str0(N_sim, 0.0);              // store city-wide home prices
	vector<double> rent_str0(N_sim, 0.0);             // store city-wide rent
	vector<double> yc_str0(N_sim, 0.0);               // city-wide income
	vector<double> yi_str0(N_sim, 0.0);               // individual income
	...
	// alternate: set log_fe equal to log of observed 30-year old households

	double y_sig_eps = pow(0.0584, 0.5);   // idiosyncratic, transitory component of labor income growth
	double y_sig_u = pow(0.0169, 0.5);     // idiosyncratic, permanent component of labor income growth

	//double y_sig_agg = pow(0.0584, 0.5);
	//double y_sig_agg = pow(0.0584, 0.5);
	...
	//double sigma_u = pow(0.0584, 0.5);  //0.005;                              // permanent income shock stdev
	//double sigma_e = pow(0.0169, 0.5); // 0.005;                              // idiosyncratic income shock stdev
	...
	// Alternate: you can also observe 30 year old income in 2005 and include this as a scaling factor relative to predicted income using $1992 

	double rep_ratio = 0.938873;                         // retirement replacement ratio

	double yc_start = 0.5;                               // starting city-wide income
	double yi_start = 0.4;                               // starting individual income
	...
	double i_age0 = age_begin;
	//double i_age0 = 30.0; // initial age

	// loop through simulations
	cout << "load_pricepath.cpp: Begin Simulations" << endl;
	for (n = 0; n < N_sim; n++){                  
		if ( n % 10000 == 0) {
			cout << "n = " << n << endl;
		}
		...
		ph_str[t][n] = log(ph0);
		//ph_str[t][n] = ph0;                     // load in initial price, rent, city-wide income, individual income
		rent_str[t][n] = rent;
		...
		// update income path
		v_t = v_t0 + u_t;  // compute permanent component
		...
		ret_tn = (csf_1yr - ph0) / ph0 + sigma_ret*dist(gen);  // impose in first year return is around the futures-based forecast
	...
	cout << "load_pricepath.cpp: Finished Running Simulations" << endl;
	cout << "load_pricepath.cpp: Compute Home Price Nodes " << endl;
	for (t = 0; t < T_sim; t++) {
		...
		for (n = 0; n < n_ph; n++) {
			ph_str_nds[t][n] = res_mean + ph_nd_std[n] * res_std;
			//(*snodes1).p_gridt[t][n] = ph_str_nds[t][n];    // load home price nodes into p_gridt
	...
	cout << "load_pricepath.cpp: Compute Rent Nodes " << endl;
	for (t = 0; t < T_sim; t++) {
	...
	cout << "load_pricepath.cpp: Compute Income Nodes " << endl;
	for (t = 0; t < T_sim; t++) {
		...
		for (n = 0; n < n_yi; n++) {
			yi_str_nds[t][n] = res_mean + yi_nd_std[n] * res_std;
			//(*snodes1).yi_gridt[t][n] = yi_str_nds[t][n];    // load individual-income nodes into income_gridt
	...
	cout << "load_pricepath.cpp: Begin Computing State-Space Transition Matrix" << endl;
	...
	cout << "load_pricepath.cpp: Cycle Through Observations" << endl;
	for (t = 0; t < T_sim; t++) {
		...
		cout << "load_pricepath.cpp: t = " << t  << "  load transition matrix into data structure "<< endl;
	...
	cout << "load_pricepath.cpp: Finished Computing Transition Matrix" << endl;

	cout << "load_pricepath.cpp: gamma T max:" << endl;
